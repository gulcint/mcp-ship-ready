...
diff` and revert with `git checkout -- <file>` if a fix is wrong. A file
whose content changed since you last ran `/mcp-ship-ready:fix` is skipped,
not overwritten — re-run `/mcp-ship-ready:fix` to refresh the preview. A
symlinked file is never written to, even if it resolves inside the target
repo. Findings categorized `architectural` are never touched by this tool
under any command.
